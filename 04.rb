...
input = (ARGV.empty? ? DATA : ARGF).readlines.sort.map { |l|
	r = /\[(.*) (.*)\] (\w*) .(\d*)/.match(l).captures
	[r[0], r[1], r[2] == "falls" ? "ASLEEP" : "AWAKE", r[3] == "" ? nil : r[3].to_i]
}
...
until input.empty?
	...
	current_date, current_time, current_state, current_id = input.shift
	...
	60.times { |tick|
		if input.length > 0 and Date.parse("#{current_date} #{current_time}") == Date.parse("#{input[0][0]} 00:#{tick.to_s.rjust(2,"0")}")
			current_date, current_time, current_state, id = input.shift
			current_id = id if id != nil
		end
		
		log[current_date][:id] = current_id if log[current_date][:id] == nil

		log[current_date][:schedule] += [current_state == "AWAKE" ? "." : "#"]
	}
end

printlog = lambda {
	log.each { |k,v|
		puts "#{k} #{v[:id].to_s.rjust(4," ")} #{v[:schedule].join('')} #{v[:schedule].count {|x| x == '#'}}"
	}
}
...
strategy1 = lambda {
	sums = Hash.new(0)
	log.each{|k,v|
		sums[v[:id]] += v[:schedule].count { |x| x == "#" } 
	}

	id = sums.max_by{|k,v| v}[0]

	schedules = []
	log.each{|k,v|
		schedules.push v[:schedule] if v[:id] == id
	}
	max_day = schedules.map { |s|
		s.map { |it|
			it == "#" ? 1 : 0 
		}
	}.transpose.map {|x| x.reduce(:+)}.each_with_index.max[1]

	puts id * max_day
}

strategy2 = lambda {
	scheduleMap = Hash.new([])
	
	log.each{|k,v|
		scheduleMap[v[:id]] += [v[:schedule]]
	}

	scheduleMap.inject(scheduleMap){ |h, (id, schedules)|
		h[id] = schedules.map { |s|
			s.map { |it|
				it == "#" ? 1 : 0 
			}
		}
